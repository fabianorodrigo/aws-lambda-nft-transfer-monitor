...
This application reads the past `Transfer` events emitted by a ERC-721 smart contract which address is defined at environment variable `CONTRACT_ADDRESS`. The search starts from the block defined at another environment variable `FROM_BLOCK`. The Ethereum provider to which the connection will be established is at environment variable `ETH_PROVIDER`.

After the events are detected they are inserted in a DynamocDB instance defined at environment variable `DYNAMODB_ENDPOINT`.
...
```bash
aws cloudformation delete-stack --stack-name nft-transfer-monitor
```
